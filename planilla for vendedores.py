...
class VentanaAbono(ctk.CTkToplevel):
    """Ventana pop-up para cargar abonos"""
...
class ModuloOTs(ctk.CTk):
    def __init__(self):
        ...
        # DATOS DE PRUEBA
        self.datos_ots = [
            {"ot": "1000", "fecha": "17/12/2025", "vendedor": "Marcos", "cliente": "Juan Pérez", "descripcion": "100 Tarjetas personales mate con barniz localizado", "monto": 250000, "pagos": [{"m": 100000, "f": "17/12"}], "pago": "Crédito", "estado": "Aprobado"},
            {"ot": "1001", "fecha": "17/12/2025", "vendedor": "Ana", "cliente": "Súper Sol", "descripcion": "Cartel lona frontal 3x2 mts con estructura y colocación", "monto": 1200000, "pagos": [{"m": 1200000, "f": "17/12"}], "pago": "Contado", "estado": "Finalizado"},
            {"ot": "1002", "fecha": "17/12/2025", "vendedor": "Marcos", "cliente": "Farmacia Vida", "descripcion": "Vinilos microperforados para fachada principal 4 vidrios", "monto": 800000, "pagos": [{"m": 400000, "f": "17/12"}], "pago": "Crédito", "estado": "Entregado"},
        ]
    ...
    def crear_planilla_izquierda(self):
        ...
        # Buscador
        self.entry_busqueda = ctk.CTkEntry(header, placeholder_text="🔍 Buscar cliente, OT o descripción...", width=300)
        self.entry_busqueda.pack(side="right")
        self.entry_busqueda.bind("<KeyRelease>", self.actualizar_tabla)

        # --- TABLA CON 2 BARRAS DE DESPLAZAMIENTO (VERTICAL Y HORIZONTAL) ---
        ...
        # Tags de Colores
        self.tabla.tag_configure("aprobado", background="#FFEBEE") # Rojo claro
        self.tabla.tag_configure("entregado", background="#FFF9C4") # Amarillo
        self.tabla.tag_configure("finalizado", background="#C8E6C9") # Verde
    ...
    def crear_detalle_derecha(self):
        # --- 3. PANEL DERECHO CON SU PROPIO SCROLL (VERTICAL) ---
        self.frame_det = ctk.CTkScrollableFrame(self, width=320, label_text="DETALLE DE LA ORDEN", 
        ...
        self.btn_entregar = ctk.CTkButton(self.frame_det, text="MARCAR ENTREGADO", height=45, fg_color="gray", state="disabled", command=lambda: self.cambiar_estado("Entregado"))
        self.btn_entregar.pack(pady=5, fill="x", padx=10)

        self.btn_finalizar = ctk.CTkButton(self.frame_det, text="FINALIZAR PEDIDO", height=45, fg_color="gray", state="disabled", command=lambda: self.cambiar_estado("Finalizado"))
    ...
    def actualizar_tabla(self, e=None):
        ...
        for d in self.datos_ots:
            if (filtro == "Todos" or d["estado"] == filtro) and \
               (busq in d["cliente"].lower() or busq in d["ot"] or busq in d["descripcion"].lower()):
                abono = sum(p['m'] for p in d['pagos'])
                tag = d["estado"].lower()
                self.tabla.insert("", "end", values=(d["ot"], d["fecha"], d["cliente"], d["descripcion"], f"{d['monto']:,} Gs.", f"{abono:,} Gs.", d["pago"], d["estado"]), tags=(tag,))

    def al_seleccionar_fila(self, e):
        sel = self.tabla.selection()
        if not sel: return
        id_ot = str(self.tabla.item(sel[0])['values'][0])
        self.ot_seleccionada = next(x for x in self.datos_ots if x["ot"] == id_ot)
